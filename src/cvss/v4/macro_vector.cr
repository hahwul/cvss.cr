module CVSS::V4
  # Lookup tables ported verbatim from FIRSTdotorg/cvss-v4-calculator
  # (BSD-2-Clause, Copyright FIRST, Red Hat, and contributors).
  #
  # `LOOKUP` maps a 6-character macro vector ("EQ1 EQ2 EQ3 EQ4 EQ5 EQ6") to
  # the macro vector's base score. Some macro combinations are infeasible
  # and are intentionally absent — callers must handle nil lookups.
  #
  # `MAX_COMPOSED` lists, for each EQ class, the maximum-severity vector
  # fragments used when computing the within-class severity distance.
  #
  # `MAX_SEVERITY` is the depth (in level-units) of each macro region;
  # multiplied by 0.1 to convert to score units.
  module MacroVectorTables
    LOOKUP = {
    ...
    # Highest-severity vectors per EQ class. Each fragment is a
    # `KEY:VAL/...` string used by the depth-distance calculation.
    EQ1_MAXES = {
      0 => ["AV:N/PR:N/UI:N/"],
    ...
    # EQ3 maxes are keyed by EQ3 then EQ6.
    EQ3_EQ6_MAXES = {
      0 => {
    ...
    MAX_SEVERITY_EQ2     = {0 => 1, 1 => 2}
    MAX_SEVERITY_EQ3_EQ6 = {
      0 => {0 => 7, 1 => 6},
      1 => {0 => 8, 1 => 8},
      2 => {1 => 10},
    }
