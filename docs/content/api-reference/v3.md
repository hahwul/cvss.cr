...
| `impact_subscore : Float64` | Scope-aware impact (the `Impact` term in the spec), floored at `0.0`. |
| `exploitability_subscore : Float64` | The `Exploitability` term. |

The Scope-Changed polynomial evaluates to `-0.21808` when nothing is impacted
(`S:C/C:N/I:N/A:N`), a case `base_score` already reports as `0.0`. `impact_subscore`
floors it so that neither it nor the `impactScore` JSON field can go negative —
the FIRST CVSS JSON Schema constrains every score field to `0..10`. Call
`CVSS::V3::Score.impact(vector)` for the raw, unclamped formula output.
